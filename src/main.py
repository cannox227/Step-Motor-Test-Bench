    ...
    with dpg.window(label="Motor settings",tag="Motor settings"):
        ...
        with dpg.group() as motor_settings:
            ...
            with dpg.group(horizontal=True) as plot_params_VT:
                dpg.add_button(label="Plot V-t", callback=plot_V())
                dpg.add_button(label="Plot T-t", callback=plot_T())
            with dpg.group(horizontal=True) as plot_params_PI:
                dpg.add_button(label="Plot P-t", callback=plot_P())
                dpg.add_button(label="Plot I-t", callback=plot_I())
...
def plot_V():
    with dpg.window(label="Motor Voltage plot",tag="Motor Voltage plot", pos=(300, 0)):
        with dpg.plot(label="Voltage - w", height=300, width=300):
            ...
            dpg.add_line_series(plot.get_xvals(), plot.get_yvals(), label="V-w", parent="y-axis-V")
...
def plot_P():
    ...
    with dpg.window(label="Motor Power plot",tag="power_plot", pos=(300, 50)):
        with dpg.plot(label="Power - w", height=300, width=300):
            dpg.add_plot_legend()
            dpg.add_plot_axis(dpg.mvXAxis, label="w")
            dpg.add_plot_axis(dpg.mvXAxis, label="P", tag="y-axis-P")
            dpg.add_line_series(plot.get_xvals(), plot.get_yvals(), label="P-w", parent="y-axis-P")


def plot_I():
    plot.plot_I()
    with dpg.window(label="Motor Current plot",tag="Motor Current plot", pos=(300, 100)):
        with dpg.plot(label="Current - w", height=300, width=300):
            dpg.add_plot_legend()
            dpg.add_plot_axis(dpg.mvXAxis, label="w")
            dpg.add_plot_axis(dpg.mvXAxis, label="I", tag="y-axis-I")
            dpg.add_line_series(plot.get_xvals(), plot.get_yvals(), label="I-w", parent="y-axis-I")
def plot_T():
    plot.plot_T()
    with dpg.window(label="Motor Torque plot",tag="Motor Torque plot", pos=(300, 150)):
        with dpg.plot(label="Torque - w", height=300, width=300):
            dpg.add_plot_legend()
            dpg.add_plot_axis(dpg.mvXAxis, label="w")
            dpg.add_plot_axis(dpg.mvXAxis, label="T", tag="y-axis-T")
            dpg.add_line_series(plot.get_xvals(), plot.get_yvals(), label="T-w", parent="y-axis-T")
